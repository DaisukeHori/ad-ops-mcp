...
6. **全921件のテストが100%合格する**（単体321件 + IT2結合200件 + IT3結合400件）

---

## 8. テスト要件

テスト計画の詳細は `docs/test-plan.md` を参照。

### 8.1 テスト規模

| テストレベル | 件数 | 内容 |
|---|---|---|
| 単体テスト (UT) | 321件 | 全62ツール × 4パターン + インフラ49件 |
| 2ツール結合 (IT2) | 200件 | ツール間のデータ受け渡し検証 |
| 3ツール以上結合 (IT3) | 400件 | 実運用フローの再現 |
| **合計** | **921件** | |

### 8.2 テスト方針

- 外部APIは一切叩かない（msw でモック化）
- 全テスト合格率 100% を合格基準とする
- コードカバレッジ Line 90% 以上、Branch 85% 以上
- CI/CD（GitHub Actions）で毎push時に自動実行
