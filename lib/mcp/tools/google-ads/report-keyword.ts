/**
 * Google Ads キーワードレポート取得ツール
 */

import { z } from "zod";
import type { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
import { searchGoogleAds } from "@/lib/platforms/google-ads/client";
import { buildKeywordReportQuery } from "@/lib/platforms/google-ads/gaql";
import { PlatformError } from "@/lib/platforms/errors";
import { getEnv } from "@/lib/config";

export function registerGoogleAdsReportKeyword(server: McpServer): void {
  server.tool(
    "google_ads_report_keyword",
    "Google Ads のキーワードレポートを日別で取得します。キーワードごとの表示回数、クリック数、費用（マイクロ単位: 1円 = 1,000,000）、コンバージョン数を確認できます。",
    {
      customerId: z
        .string()
        .optional()
    ...
    async (params) => {
      try {
        ...
        const query = buildKeywordReportQuery({
